...
run_test_codex_metadata() {
  ...
  [[ -s "$REPO_ROOT/AGENTS.md" ]] || {
    echo "FAIL: missing root AGENTS.md" >&2
    exit 1
  }
  [[ -s "$REPO_ROOT/skills/AGENTS.md" ]] || {
    echo "FAIL: missing skills/AGENTS.md" >&2
    exit 1
  }
...
assert codex["name"] == claude["name"], "plugin names differ"
assert codex["version"] == claude["version"], "plugin versions differ"
assert codex["skills"] == "./skills/", "Codex manifest must point to ./skills/"
assert len(codex["interface"]["defaultPrompt"]) <= 3, "too many default prompts"
assert all(len(item) <= 128 for item in codex["interface"]["defaultPrompt"]), "default prompt too long"
assert (root / "skills").is_dir(), "skills directory missing"
PY
}
